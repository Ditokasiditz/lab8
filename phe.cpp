...
void draw_ship(int x, int y, int color)
{
	...
	consoleBuffer[ship.X + 2 + screen_x * ship.Y].Char.AsciiChar = 'O';
...
void check_attack_star()
{
	for (int i = 0; i < scount; i++)
	{
		if ((star[i].X == ship.X || star[i].X == ship.X + 1 || star[i].X == ship.X + 2 || 
			star[i].X == ship.X + 3 || star[i].X == ship.X + 4) && star[i].Y == ship.Y)
		{
			star[i] = {SHORT(rand() % screen_x), 1};
			check++;
		}
